...
void divert_set_charge_rate(int new_charge_rate)
{
  const int current_charge_rate = charge_rate;

  if(new_charge_rate < min_charge_current && OPENEVSE_STATE_SLEEPING != state)
  {
    if(divert_active && divertmode_get_time() >= min_charge_end)
    {
      ...
      // We just turned on, so avoid cycling the relay; set minimum rate instead
      new_charge_rate = min_charge_current;
    }
  }
...
void divert_update_state()
{
  ...
  if (enabled)
  {
    ...
    DBUGVAR(charge_rate);
    divert_set_charge_rate(charge_rate);
    event["divert_active"] = divert_active;
